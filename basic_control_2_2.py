    ...
    ####put data 
        ...
        if (r.move==1):
            ...
            if r.anga>0:
                ...
                sr=serl(int(97),int(jt.jx/10),int(r.angb),int(r.angc),int(-1*r.angd))
                #sr.serial_write_ang(int(97),int(jt.jx/10),int(r.angb),int(r.angc),int(-1*r.angd))
...
rm=Frame(root,height="800",width="1000",bg="#888")
rm.pack()

lmain = tk.Label(root)
lmain.place(x=300,y=0)
...
def show_frame():
    ...
    if ex == 0:
       ...
       os.environ['SDL_WINDOWID'] = str(rm.winfo_id())
    global m_x ,m_y
    #embed = tk.Frame(root, width = 200, height = 500) #creates embed frame for pygame window
...
#thread.start_new_thread( op, () )
#thread.start_new_thread( show_frame, () )
