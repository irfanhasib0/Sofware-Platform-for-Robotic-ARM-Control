...
####mouse 
    ...
    cv2.putText(window,'jx :%d'%int(jx),(640,140), font,.6,(0,255,0),1)
    cv2.putText(window,'jx :%d'%int(jy),(640,160), font,.6,(0,255,0),1)
    
    t5=time.time()
    cv2.putText(window,'time :%d'%int(t5*1000-t4*1000),(640,180), font,.6,(0,255,0),1)
    ...
    if (move==1):
        ...
        if anga>0:
           ...
           # serial_write_ang(int(anga),int(angb),int(angc),int(-1*angd),int(jz))
            serial_write_ang(int(97),int(jx/10),int(angb),int(angc),int(-1*angd))
            #ser.write("bhfghdfghdhjg\r")
